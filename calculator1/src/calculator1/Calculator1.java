...
( JTextField .RIGHT );
 ...
 ActionListener numberListener = new
NumberListener ();
...
++) {
 String key = buttonOrder .substring ( i, i
+ 1 );
 if ( key.equals ( " " )) {
...
BorderLayout . CENTER);
 ...
 class OperatorListener implements
ActionListener {
...
( "sin" ))
 {
 textfield .setText( "" + Math .sin
( Double .valueOf ( displayText ). doubleValue ()));

 }else
 if ( e.getActionCommand ().equals
( "cos" ))
 {
 textfield .setText( "" + Math .cos
( Double .valueOf ( displayText ). doubleValue ()));
...
( "C" ))
 ...
 class NumberListener implements
ActionListener {
 public void actionPerformed
( ActionEvent event ) {
 String digit =
